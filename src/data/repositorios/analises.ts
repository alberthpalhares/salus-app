...
import { Analise } from '../../types/dominio';
import { handleFirestoreError, OperationType } from './base';

function getCollectionPath(uid: string): string {
  return `usuarios/${uid}/analises`;
}

export async function listar(uid: string): Promise<Analise[]> {
  const path = getCollectionPath(uid);
  try {
    const snapshot = await getDocs(collection(db, path));
    return snapshot.docs.map((d) => ({ id: d.id, ...d.data() } as Analise));
  } catch (error) {
    handleFirestoreError(error, OperationType.LIST, path);
  }
}

export async function obterPorId(uid: string, id: string): Promise<Analise | null> {
  const path = `${getCollectionPath(uid)}/${id}`;
  try {
    const snap = await getDoc(doc(db, path));
    if (!snap.exists()) return null;
    return { id: snap.id, ...snap.data() } as Analise;
  } catch (error) {
    handleFirestoreError(error, OperationType.GET, path);
  }
}

export async function listarPorMembro(uid: string, membroId: string): Promise<Analise[]> {
  const path = getCollectionPath(uid);
  try {
...
export async function salvar(uid: string, dado: Analise): Promise<void> {
