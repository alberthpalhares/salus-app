...
import { ProfissionalSaude } from '../../types/dominio';
import { handleFirestoreError, OperationType } from './base';

function getCollectionPath(uid: string): string {
  return `usuarios/${uid}/profissionais`;
}

export async function listar(uid: string): Promise<ProfissionalSaude[]> {
  const path = getCollectionPath(uid);
  try {
    const snapshot = await getDocs(collection(db, path));
    return snapshot.docs.map((d) => ({ id: d.id, ...d.data() } as ProfissionalSaude));
  } catch (error) {
...
export async function obterPorId(uid: string, id: string): Promise<ProfissionalSaude | null> {
  const path = `${getCollectionPath(uid)}/${id}`;
  try {
    const snap = await getDoc(doc(db, path));
    if (!snap.exists()) return null;
    return { id: snap.id, ...snap.data() } as ProfissionalSaude;
  } catch (error) {
...
export async function salvar(uid: string, dado: ProfissionalSaude): Promise<void> {
